...
export const DepartmentModal = ({ isOpen, onClose, onSave, initialData, isDark }: DepartmentModalProps) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({ userId:'', name: '',  blockId: '', });
    }
  }, [initialData, isOpen]);
        ...
        >
          ...
          <div>
            ...
            <select
              required
              className={cn(
                "w-full px-4 py-2 rounded-lg border outline-none focus:ring-2 focus:ring-blue-500/20 transition-all disabled:cursor-not-allowed",
                isDark
                  ? "bg-black/20 border-white/10 text-white"
                  : "bg-gray-50 border-gray-200"
              )}
